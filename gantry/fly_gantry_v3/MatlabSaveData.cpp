	...
	matFileName(matFileName)
{
	...
	onPROJ.reserve(vectorSizeToStart);
...
void MatlabSaveData::addFrameData(flyViewStruc& fvStruc)
{
	...
	onPROJ.push_back(static_cast<unsigned char>(fvStruc.onPROJ));
...
{	
	...
	if (!pmat)
	{
		std::cerr << "Failed to open MAT file for writing: " << matFileName << std::endl;
		return;
	}
	...
	if (numberOfFrames == 0)
	{
		std::cout << "MAT save warning: no frame data collected yet for " << matFileName << std::endl;
	}
