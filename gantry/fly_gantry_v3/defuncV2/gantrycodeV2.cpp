...
2, Spinnakr SDK library for the pointgrey camera(top)
...
struct BottomCamContext
{
	bool set_for_exp = true;
	bool allowBottomCamToGo = true;
	bool topCamFlyFound = false;
	Point2f flyPos_encoder = Point2f(0.0f, 0.0f);
	Point2f spacer_center_encoder = Point2f(0.0f, 0.0f);
	long long arena_radius = 0;
	Point2f bottomCamRecentFlyPos = Point2f(0.0f, 0.0f);

	void setFlyMouseCallbackFunc(void*) {}
	void getTransformationMatrix() {}
	void turnOnSaveIndicator() {}
	void turnOffSaveIndicator() {}
};

static BottomCamContext arenaP;
...
int main(int argc, char* argv[]) // argc: argument count, argv: argument vector
{
	...
	// Encoder source has moved to Zaber. Position is polled from the stage directly.
	...
	//----- Build 5 Parallel Sections----//
	// Five sections: CALIBRATING BOTTOM CAMERA, Fly tracking with yolo, Driving the motors(Controling part), Disply bottom view, Save Data
...
#pragma omp section

		{
			...
			using namespace Basler_UsbCameraParams; // Basler USB camera
			...
			try
			{
				...
				bool encoderAcquisitionInitialized = false;
				auto lastEncoderSample = std::chrono::steady_clock::now();

				// Capture frames and get stage positions directly from Zaber.
				while (TRUE)
				{
					if (!encoderAcquisitionInitialized)
					{
						cout << "XY calibrated. Setting up Zaber position acquisition" << endl;
						motors.startDataAcquisition();
						arenaP.set_for_exp = true;
						arenaP.setFlyMouseCallbackFunc(&arenaP);
						arenaP.getTransformationMatrix();
						encoderAcquisitionInitialized = true;
					}

					const Point2f encoderPosMm = motors.GetPosition();
					encoderX = static_cast<unsigned long>(std::max(0.0f, encoderPosMm.x) * 1000.0f);
					encoderY = static_cast<unsigned long>(std::max(0.0f, encoderPosMm.y) * 1000.0f);

					auto now = std::chrono::steady_clock::now();
					auto elapsedNs = std::chrono::duration_cast<std::chrono::nanoseconds>(now - lastEncoderSample).count();
					lastEncoderSample = now;
					if (elapsedNs <= 0)
					{
						elapsedNs = static_cast<long long>(correctdt);
					}
					dt = static_cast<unsigned long>(elapsedNs);

					encoderData encoderDataInst;
					encoderDataInst.encoderX = encoderX;
					encoderDataInst.encoderY = encoderY;
					encoderDataInst.dt = dt;
					encoderDataInst.dataNum = ++encCnt;
					encoderQ.push(encoderDataInst);

					if (wnd.keyPressed(VK_NUMPAD5))
					{
						cout << "encoder X: " << endl;
						cout << encoderX << endl;
						cout << "encoder Y: " << endl;
					...
					if (dt > correctdt * 1.5 || dt < correctdt * 0.5)
					{
					...
					//	Legacy decoder output write removed
...
#pragma omp section
		//FLY TRACKING WITH YOLO
		{
			...
			// Top-camera calibration removed in bottom-camera-only mode.
			...
			while (TRUE)
			{
				// Top-camera calibration key path removed.
				...
				if (!flyViewQ.empty() && !encoderQ.empty())
				{
					...
					if (arenaP.set_for_exp) {
						...
						// Opto stimulation is disabled in the current hardware/software version.
					}
					else
					{
						filter.Predict();
					...
					if (doSave)
					{
						...
						chunkStrucSave.onLED = 0;
						chunkStrucSave.center[0] = 0;
						chunkStrucSave.center[1] = 0;
						...
						chunkStrucSave.stimulusVoltage = 0;
					...
					if (false && arenaP.set_for_exp)
					{
						...
						(void)calibrationSquareEncoderLocation;
