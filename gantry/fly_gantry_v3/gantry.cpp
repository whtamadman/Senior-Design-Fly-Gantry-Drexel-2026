...
namespace ZaberGantry {
    ...
    namespace {
        constexpr float kAutomaticDeadZonePx = 25.0f;
    }
    ...
    ZaberGantry::Gantry::Gantry(const std::string& comPort) {
        ...
        if (connectionDevice.empty()) {
            throw std::runtime_error("No Zaber devices detected on " + comPort);
        }
        device = std::make_shared<Device>(connectionDevice.at(0));
        ...
        maxSpeed = 60.0; maxAcceleration = 1000.0; // Max limit for velocity and acceleration mm/s and mm/s^2
        maxOutput = maxSpeed;
        fps = 60; // To calculate actual velocity
    ...
    void Gantry::SetVelocity(cv::Point2f pt, cv::Point2f vel, bool automatic)
    {
        ...
        if (automatic) {
            const float errorMagPx = std::sqrt(pt.x * pt.x + pt.y * pt.y);
            if (errorMagPx <= kAutomaticDeadZonePx) {
                velX = 0;
                velY = 0;
            }
            else {
	...
	void Gantry::SetPosition(cv::Point2f pt) {

		double posX = pt.x;
		double posy = pt.y;

		xAxis.moveAbsolute(posX, posUnit);
		yAxis.moveAbsolute(posy, posUnit);
	...
	void Gantry::StopVelocity() {
		xAxis.moveVelocity(0, velUnit);
		yAxis.moveVelocity(0, velUnit);
	}
